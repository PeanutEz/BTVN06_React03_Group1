...
const ShiftPage = () => {
  ...
  const load = useCallback(
    ...
    [searchName, filterFranchise, currentPage, showDeleted],
  );
  useEffect(() => {
    ...
    void load("", initFranchise, 1);
    void loadFranchises();
  }, [load, managerFranchiseId]);
  // Sync khi managerFranchiseId thay đổi (store hydrate muộn)
  useEffect(() => {
    if (!managerFranchiseId) return;
    setFilterFranchise(managerFranchiseId);
    void load(searchName, managerFranchiseId, 1, showDeleted);
  }, [managerFranchiseId, load, searchName, showDeleted]);
