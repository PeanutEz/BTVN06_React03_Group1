...
function resolveCustomerLoyaltyPoints(customer: ApiCustomer): number | undefined {
  const record = customer as Record<string, unknown>;

  const direct =
    toFiniteNumber(record.loyalty_points) ??
    toFiniteNumber(record.loyalty_point);
  if (direct != null) return direct;

  const franchises = Array.isArray(record.franchises)
    ? (record.franchises as Array<Record<string, unknown>>)
    : [];

  for (const item of franchises) {
    const points =
      toFiniteNumber(item.loyalty_points) ??
      toFiniteNumber(item.loyalty_point);
...
function resolveCustomerLoyaltyTier(customer: ApiCustomer): string | undefined {
  const record = customer as Record<string, unknown>;
  if (typeof record.loyalty_tier === "string" && record.loyalty_tier.trim()) {
    return record.loyalty_tier.trim();
  }
  ...
  for (const item of franchises) {
    if (typeof item.loyalty_tier === "string" && item.loyalty_tier.trim()) {
      return item.loyalty_tier.trim();
    }
  }
...
function toPOSCustomerSearchResult(customer: ApiCustomer): POSCustomerSearchResult {
  return {
    id: customer.id,
    name: customer.name,
    ...
    loyalty_points: resolveCustomerLoyaltyPoints(customer),
    loyalty_tier: resolveCustomerLoyaltyTier(customer),
...
export const posService = {
  ...
  searchCustomer: async (keyword: string): Promise<POSCustomerSearchResult[]> => {
    ...
    try {
      ...
      return (response.data.data || []).map(toPOSCustomerSearchResult);
    } catch {
      ...
      return (fallbackResponse.data.data || []).map(toPOSCustomerSearchResult);
    }
  },
